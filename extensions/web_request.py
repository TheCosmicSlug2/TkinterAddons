...
class InternetRequests:
    ...
    def get_image_from_url(image_url: str) -> PIL.Image:
        ...
        if reponse_object.status_code != 200:
            print(f"Échec du chargement de l'image depuis l'URL \"{image_url}\".\n" 
                  f"Code d'état HTTP : \"{reponse_object.status_code}\"")
            return
    ...
    @staticmethod
    def get_image_streams(yt_object: pytube.YouTube, quality: str):
        streams = yt_object.streams.filter(adaptive=True, only_video=True)
        if quality == "low":
            return streams[0]
        return streams[-1]

    @staticmethod
    def get_audio_streams(yt_object: pytube.YouTube, quality: str):
        streams = yt_object.streams.filter(adaptive=True, only_audio=True)
        if quality == "low":
            return streams[0]
        return streams[-1]
